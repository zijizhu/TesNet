...
class CUBEvalDataset(ImageFolder):
    def __init__(self,
        ...
        path_df = pd.read_csv(root / "CUB_200_2011" / "images.txt", header=None, names=["image_id", "image_path"], sep=" ")
        bbox_df = pd.read_csv(root / "CUB_200_2011" / "bounding_boxes.txt", header=None, names=["image_id", "x", "y", "w", "h"], sep=" ")
    ...
    def __getitem__(self, index: int):
        ...
        seg_map = read_image((self.segmentation_map_root / class_dir / sample_filename.replace("jpg", "png")).as_posix())
...
def evaluate_comprehensiveness(net: nn.Module,
    ...
    eval_dataset = CUBEvalDataset(root="datasets", transform=transform)
    eval_dataloader = DataLoader(dataset=eval_dataset, batch_size=64, num_workers=2, shuffle=True)

    net.to(device)
    net.eval()

    binary_thresh_to_IoUs = defaultdict(list)
    for b, batch in enumerate(tqdm(eval_dataloader)):
        ...
        for thresh in thresholds:
            binary_thresh_to_IoUs[thresh] += batch_IoU_binary(batch_activations_resized, seg_map_cropped, threshold=thresh)
        
    binary_thresh_to_scores = defaultdict(float)
    for thresh, mean_IoUs in binary_thresh_to_IoUs.items():
        score = sum(mean_IoUs) / len(mean_IoUs)
